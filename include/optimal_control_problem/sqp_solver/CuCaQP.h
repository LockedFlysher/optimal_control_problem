 ...
 */
inline Eigen::SparseMatrix<OSQPFloat> casadiDMToEigenSparse(const casadi::DM &casadiMatrix);
 ...
 */
inline Eigen::Matrix<OSQPFloat , Eigen::Dynamic, 1> casadiDMToEigenVector(const casadi::DM &casadiVector);
 ...
 * @return 转换后的Eigen稀疏矩阵
 */
inline Eigen::SparseMatrix<OSQPFloat> torchTensorToEigenSparse(const torch::Tensor &torchMatrix);
 ...
 * @return 转换后的Eigen密集向量
 */
inline Eigen::Matrix<OSQPFloat , Eigen::Dynamic, 1> torchTensorToEigenVector(const torch::Tensor &torchVector);
...
inline Eigen::SparseMatrix<OSQPFloat> casadiDMToEigenSparse(const casadi::DM &casadiMatrix) {
    ...
    Eigen::SparseMatrix<OSQPFloat> eigenMatrix(rows, cols);
    std::vector<Eigen::Triplet<OSQPFloat>> triplets;
    ...
    for (casadi_int j = 0; j < sparsity.size2(); ++j) {
        for (casadi_int k = colind[j]; k < colind[j + 1]; ++k) {
            ...
            triplets.push_back(Eigen::Triplet<OSQPFloat>(i, j, data[k]));
        }
    }
...
inline Eigen::Matrix<OSQPFloat, Eigen::Dynamic, 1> casadiDMToEigenVector(const casadi::DM &casadiVector) {
    ...
    // 创建 Eigen 向量
    Eigen::Matrix<OSQPFloat, Eigen::Dynamic, 1> eigenVector(size);

    // 填充数据
    for (int i = 0; i < size; i++) {
        eigenVector(i) = casadiVector(i).scalar();
    }
    return eigenVector;
}
...
inline Eigen::SparseMatrix<OSQPFloat> torchTensorToEigenSparse(const torch::Tensor &torchMatrix) {
    ...
    if (cpuTensor.is_sparse()) {
        ...
        OSQPFloat* valuesPtr = values.data_ptr<OSQPFloat>();

        int nnz = values.size(0);
        std::vector<Eigen::Triplet<OSQPFloat>> triplets(nnz);

        for (int k = 0; k < nnz; ++k) {
            int i = indicesPtr[k];
            int j = indicesPtr[k + nnz]; // 第二行索引
            triplets[k] = Eigen::Triplet<OSQPFloat>(i, j, static_cast<double>(valuesPtr[k]));
        }
...
inline Eigen::Matrix<OSQPFloat, Eigen::Dynamic, 1> torchTensorToEigenVector(const torch::Tensor &torchVector) {
    ...
    if (cpuTensor.dtype() == torch::kFloat32) {
        OSQPFloat* dataPtr = cpuTensor.data_ptr<OSQPFloat>();
        Eigen::Map<Eigen::Matrix<OSQPFloat, Eigen::Dynamic, 1>> eigenMap(dataPtr, size);
        eigenVector = eigenMap.cast<OSQPFloat>();
    }
    else if (cpuTensor.dtype() == torch::kFloat64) {
        OSQPFloat* dataPtr = cpuTensor.data_ptr<OSQPFloat>();
        Eigen::Map<Eigen::Matrix<OSQPFloat, Eigen::Dynamic, 1>> eigenMap(dataPtr, size);
        eigenVector = eigenMap;
    }
    else {
        // 对于其他数据类型，使用逐元素复制
        auto accessor = cpuTensor.accessor<float, 1>();
        for (int i = 0; i < size; ++i) {
            eigenVector(i) = static_cast<OSQPFloat>(accessor[i]);
        }
    }
